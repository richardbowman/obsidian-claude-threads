...
- `traces.listSources/readChunk/subscribe` returns immutable, bounded, sanitized semantic records through opaque cursors. Source discovery is paged; chunks use byte-offset cursors bound to the source ID and a revision that changes if the log is replaced or truncated. `contentHash` is an incremental metadata fingerprint that changes when log content is appended without scanning the whole log. `TraceEvent.invokedSkill` is present only when a structured assistant `Skill` request names a currently registered skill and has a correlated, successful `tool_result`; failed, rejected, unregistered, and text-only claims remain unattributed. Raw log paths, credentials, and known secret fields are never returned.
