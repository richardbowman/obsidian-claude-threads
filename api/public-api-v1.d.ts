...
export interface TraceSource { readonly sourceId: string; readonly threadId: string; readonly projectId?: string; readonly harness: AgentHarness; readonly revision: string; readonly contentHash: string; readonly updatedAt: number }
...
export interface AgentThreadsApiV1 {
  ...
  readonly threads: { list(query?: { readonly projectId?: string | null; readonly status?: string; readonly limit?: number }): Promise<readonly ThreadSummary[]>; get(threadId: string): Promise<ThreadSnapshot | null>; create(input: CreateThreadInput): Promise<{ readonly threadId: string }>; send(threadId: string, input: SendInput): Promise<{ readonly runId: string }>; wait(runId: string, options?: { readonly timeoutMs?: number }): Promise<RunResult>; cancel(runId: string): Promise<Exclude<RunResult, { status: 'timed_out' }>>; open(threadId: string): Promise<void>; subscribe(listener: (event: unknown) => void): Disposable };
  readonly traces: { listSources(): Promise<readonly TraceSource[]>; readChunk(sourceId: string, options?: { readonly cursor?: string; readonly limit?: number }): Promise<TraceChunk>; subscribe(listener: (event: unknown) => void): Disposable };
  ...
  readonly agentTools: { createBundle(profile: 'voice-orchestration'): { readonly tools: readonly unknown[]; execute(name: string, args: Record<string, unknown>): Promise<string> } };
}
